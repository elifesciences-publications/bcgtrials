...
single_runIndivSORI <- function(vacc_target,retain_reactors,vacc_effS,vacc_effI,DIVA, Immunity=1.0/1.650)
{
trial_duration = 5
batch_interval = 180
...
1.0-vacc_effS,1.0,1,
params[5],params[12],Immunity,1.0,params[5],params[5],params[12],
...
outlist <- mclapply(1:reps,function(x){single_runIndivSORI(0.0,0,0.0,0.0,DIVA1,3*(1.0/1.650))},mc.set.seed=TRUE,mc.preschedule=FALSE,mc.cores=4) 
SimsDf = rbind(SimsDf,cbind(as.data.frame(do.call(rbind,outlist)),immune=3*(1.0/1.650),remove=T,treatment=F))

outlist <- mclapply(1:reps,function(x){single_runIndivSORI(0.0,0,0.0,0.0,DIVA1,6*(1.0/1.650))},mc.set.seed=TRUE,mc.preschedule=FALSE,mc.cores=4) 
SimsDf = rbind(SimsDf,cbind(as.data.frame(do.call(rbind,outlist)),immune=6*(1.0/1.650),remove=T,treatment=F))
