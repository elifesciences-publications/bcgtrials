...
p2=(ggplot(dfE,aes(x=Herds,y=power,col=measure,linetype=VE,shape=measure)) + geom_line()  +  scale_x_continuous('Herds',limits=c(51,300)) + scale_y_continuous('Efficacy',limits=c(-0.2,1))) + geom_point() + scale_colour_manual(values=cbPaletteB)+ guides(linetype = guide_legend(order=1,title=expression(epsilon[I])),colour=guide_legend(order=2,title='Measures'),shape=guide_legend(order=2,title='Measure'))

p1=(ggplot(df,aes(x=Herds,y=power,col=measure,linetype=VE,shape=measure)) + geom_line()  +  scale_x_continuous('Herds',limits=c(51,300)) + scale_y_continuous('Power'))+ geom_point() + scale_colour_manual(values=cbPaletteB)+ guides(linetype = guide_legend(order=1,title=expression(epsilon[I])),colour=guide_legend(order=2,title='Measures'),shape=guide_legend(order=2,title='Measure'))
