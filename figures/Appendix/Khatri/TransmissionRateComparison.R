...
legend('topleft',c('Posterior','MLE'),col=c('black'),lwd=2,lty=c(1,3))

plot(density((betaPred10*10)),col='black',lwd=2,xlab=expression(beta),main='Field Estimates (ABC)',xlim=c(0,0.0025))
lines(density((betaPred100*100)),col='orange',lwd=2,lty=2)
abline(v= (exp(mleTrans)),lty=2,lwd=2)
legend('topright',c('SOR (H=10)','SOR (H=100)'),col=c('black','orange'),lwd=2,lty=c(1,3))
dev.off()
