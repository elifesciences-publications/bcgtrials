...
p3=(ggplot(df60,aes(x=Herds,y=100*power,linetype=VEI,col=coverage)) + geom_line()  +  scale_x_continuous('Herds') + scale_y_continuous('Power',limits=c(0.0,100)))+ guides(linetype = guide_legend(order=1,title=expression(epsilon[I])),colour=guide_legend(order=2,title='Coverage'),shape=guide_legend(order=2,title='Coverage'))+ scale_colour_manual(values=cbPaletteB) + geom_point(size=0.75)

p4=(ggplot(df60,aes(x=Herds,y=100*median,ymin=lower,ymax=upper,linetype=VEI,col=coverage)) + geom_line()  +  scale_x_continuous('Herds') + scale_y_continuous('% Effect Size',limits=c(0,20)))+ guides(linetype = guide_legend(order=1,title=expression(epsilon[I])),colour=guide_legend(order=2,title='Coverage'),shape=guide_legend(order=2,title='Coverage'))+ scale_colour_manual(values=cbPaletteB) + geom_point(size=0.75)

p5=(ggplot(df90,aes(x=Herds,y=100*power,linetype=VEI,col=coverage,shape=coverage)) + geom_line()  +  scale_x_continuous('Herds') + scale_y_continuous('Power',limits=c(0.0,100)))+ guides(linetype = guide_legend(order=1,title=expression(epsilon[I])),colour=guide_legend(order=2,title='Coverage')) + scale_colour_manual(values=cbPaletteB) + geom_point(size=0.75)
