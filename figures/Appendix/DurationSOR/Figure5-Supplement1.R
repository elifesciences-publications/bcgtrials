...
save.image('PaperFigure5-supplement1.RData')
...
p2=(ggplot(df30,aes(x=Herds,y=100*median,ymin=lower,ymax=upper,linetype=VEI,col=coverage,shape=coverage)) + geom_line() + geom_point(size=0.75)  +  scale_x_continuous('Herds') + scale_y_continuous('% Effect Size',limits=c(0,20)))+ guides(linetype = guide_legend(order=1,title=expression(epsilon[I])),colour=guide_legend(order=2,title='Coverage'))+ scale_colour_manual(values=cbPaletteB)
...
pdf('Figure-Duration-Power-SOR.pdf',height=2.5*4,width=8.5,family='Helvetica',pointsize=12)
print(power_plot)
dev.off()

pdf('Figure-Duration-Effect-SOR.pdf',height=2.5*2,width=2.5*2,family='Helvetica',pointsize=12)
