...
control <- (ICDIVA1df$Reactors > 0 | ICDIVA1df$Slaughter > 0)
casesW <- (VES30VEI0IVP100DIVA1df$Reactors > 0 | VES30VEI0IVP100DIVA1df$Slaughter > 0)  
casesM <- (VES30VEI0IVP50DIVA1df$Reactors > 0 | VES30VEI0IVP50DIVA1df$Slaughter > 0)
...
control <- (ICDIVA1df$Reactors > 0 | ICDIVA1df$Slaughter > 0)
casesW <- (VES30VEI30IVP100DIVA1df$Reactors > 0 | VES30VEI30IVP100DIVA1df$Slaughter > 0)  
casesM <- (VES30VEI30IVP50DIVA1df$Reactors > 0 | VES30VEI30IVP50DIVA1df$Slaughter > 0)
...
control <- (ICDIVA1df$Reactors > 0 | ICDIVA1df$Slaughter > 0)
casesW <- (VES30VEI60IVP100DIVA1df$Reactors > 0 | VES30VEI60IVP100DIVA1df$Slaughter > 0)  
casesM <- (VES30VEI60IVP50DIVA1df$Reactors > 0 | VES30VEI60IVP50DIVA1df$Slaughter > 0)
...
control <- (ICDIVA1df$Reactors > 0 | ICDIVA1df$Slaughter > 0)
casesW <- (VES30VEI90IVP100DIVA1df$Reactors > 0 | VES30VEI90IVP100DIVA1df$Slaughter > 0)  
casesM <- (VES30VEI90IVP50DIVA1df$Reactors > 0 | VES30VEI90IVP50DIVA1df$Slaughter > 0)
...
p1=(ggplot(df,aes(x=Herds,y=100*power,linetype=VEI,col=coverage)) + geom_line()  +  scale_x_continuous('Herds') + scale_y_continuous('Power',limits=c(0.0,100)))+ guides(linetype = guide_legend(order=1,title=expression(epsilon[I])),colour=guide_legend(order=2,title='Coverage'))


p2=(ggplot(df,aes(x=Herds,y=100*median,ymin=lower,ymax=upper,linetype=VEI,col=coverage)) + geom_line()  +  scale_x_continuous('Herds') + scale_y_continuous('% Effect Size',limits=c(0,15)))+ guides(linetype = guide_legend(order=1,title=expression(epsilon[I])),colour=guide_legend(order=2,title='Coverage'))

pdf('VES30PowerIncidencebyVI.pdf',height=2.5,width=6.5,family='Helvetica',pointsize=12)
grid.newpage()
